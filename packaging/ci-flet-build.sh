#!/usr/bin/env bash
# Hardened Flet desktop build for CI: preflight template download, retry with --clear-cache.
set -euo pipefail
...
preflight_template() {
  echo "Preflight: Flet build template v${FLET_VERSION}..."
  for attempt in 1 2 3 4 5; do
    if curl -fSL --retry 5 --retry-delay 5 --retry-all-errors \
        -o "$TEMPLATE_ZIP" "$TEMPLATE_URL" \
        && valid_template_zip; then
      echo "Preflight OK ($(wc -c < "$TEMPLATE_ZIP" | tr -d '[:space:]') bytes)"
      return 0
    fi
    echo "Template preflight failed (attempt ${attempt}/5)" >&2
    rm -f "$TEMPLATE_ZIP"
    sleep "$((attempt * 5))"
  done
  echo "GitHub template asset unavailable: $TEMPLATE_URL" >&2
  return 1
}
