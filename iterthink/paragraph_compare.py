...
from iterthink.paragraph_align import (
    DiffParagraph,
    compute_alignment,
    compute_hash,
    para_id_for,
...
async def refine_alignment_diffs_duplicate_hash_async(
    ...
    for _h, group in by_hash.items():
        ...
        try:
            emb_o = await embed_texts(embed_client, embed_model, texts_o)
            emb_n = await embed_texts(embed_client, embed_model, texts_n)
    ...
    *,
    ...
    try:
        diffs = await refine_alignment_diffs_duplicate_hash_async(
            embed_client, embed_model, old_paras, new_paras, diffs
        )
    except BaseException:
        pass
