...
from iterthink.services.rag.context_format import chunk_usable_for_norm_context, rag_chunk_display_body


def test_rejects_trivial_heading_chunk() -> None:
    assert not chunk_usable_for_norm_context("### 0")
    assert not chunk_usable_for_norm_context("### –")


def test_rejects_short_chunk() -> None:
    assert not chunk_usable_for_norm_context("### hi")
...
def test_rejects_toc_dot_leaders() -> None:
    ...
    assert not chunk_usable_for_norm_context(toc)
...
def test_accepts_substantive_excerpt() -> None:
    ...
    assert chunk_usable_for_norm_context(s)
...
def test_rag_context_wrapper_stripped_for_display() -> None:
    ...
    assert chunk_usable_for_norm_context(wrapped)
...
def test_rag_context_wrapper_body_still_rejected_if_junk() -> None:
    ...
    assert not chunk_usable_for_norm_context(wrapped)
