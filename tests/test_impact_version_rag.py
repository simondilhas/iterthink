"""Version-scoped Impact RAG: MAX(content_version_id) per lineage."""
...
def test_fetch_latest_rows_prefers_max_version_per_lineage() -> None:
    ...
    try:
        ...
        emb[0] = 1.0
        dk_v1 = impact_rag.doc_key_version(lid, 10)
        dk_v2 = impact_rag.doc_key_version(lid, 20)
        ...
        rows = store_db.impact_version_chunk_fetch_latest_rows(conn, [lid])
        assert len(rows) == 1
        assert rows[0][1] == 20
        ...
        txt = impact_rag.retrieve_context_by_lineage_ids(
            q, conn, [lid], {lid: "doc1.md"}, top_k=1
        )
        assert "NEW" in txt
